...
import { DomainServiceEditor } from "@/components/admin/domain/domain-service-editor";
import {
  getDictionaryBySlug,
...
function normalizeDetails(
  row: DomainServiceDetails | null,
): DomainServiceDetails | null {
  if (!row) return null;
  const faq = Array.isArray(row.faq)
    ? (row.faq as DomainFaqItem[])
    : [];
  return { ...row, faq };
}

export default async function EditServicePage({ params }: Props) {
  const { dictionarySlug, id } = await params;
  ...
  if (!service || service.dictionary_id !== dictionary.id) notFound();
    ...
    return (
      <DomainServiceEditor
        ...
        details={normalizeDetails(
          detailsError ? null : (details as DomainServiceDetails | null),
        )}
      />
    );
  }

  const [{ data: categories }, draft] = await Promise.all([
    supabase
      .from("categories")
      .select(
    ...
    ensureServiceDraft(supabase, id),
  ]);
